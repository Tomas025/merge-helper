...
export async function runMockMergeForPR(args: {
  ...
  await execa("git", ["config", "user.name", "sesame-merge-helper"], withOpts(srcDir));
  ...
  const workBranch = `sesame-mock/${args.prNumber}-${args.headSha.slice(0, 7)}`;
  ...
  await execa("git", ["commit", "-m", "sesame-mock: resolução automática (mock)"], withOpts(srcDir));
...
export async function applyResolvedCommit(args: {
  ...
  if (!(await fs.pathExists(metaPath))) {
    return { ok: false as const, message: "Workspace não encontrado; reexecute o mock" };
  }
  ...
  try {
    ...
    await fs.rm(path.join(os.tmpdir(), "sesame-merge-helper-diffs", `${safeKey}.html`), { force: true }).catch(()=>{});
    ...
    return { ok: false as const, message: "Base mudou; reexecute o mock para recalcular a resolução", log: e.stderr || e.message };
  }
}
